; * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * *
;					  	PROJETO GRUPO KAMINARI                      	  *	
;			CONTROLADOR DE BANHO POR TEMPO COM VISOR LCD                  *
; * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * *
...
; * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * *
 ...
 __CONFIG _CP_OFF & _CPD_OFF & _DEBUG_OFF & _LVP_OFF & _WRT_OFF & _BODEN_OFF & _PWRTE_ON & _WDT_ON & _HS_OSC
...
;  ESTE BLOCO DE VARIÁVEIS ESTÁ LOCALIZADO LOGO NO INÍCIO DO BANCO 0
	...
	CBLOCK	0X20			; POSIÇÃO INICIAL DA RAM
		...
		QTDE_LITROS			 ; CONTAGEM DOS LITROS
...
#DEFINE	TELA_DE_STANDBY	FLAG,1


#DEFINE	TELA_DE_AGRADECIMENTO	FLAG,2


#DEFINE	MOSTRA_RPS	FLAG,7		; FLAG PARA MOSTRAR A ROTAÇÃO NO LCD
					; 1 -> DEVE MOSTRAR A ROTAÇÃO
					; 0 -> NAO DEVE MOSTRAR A ROTAÇÃO

#DEFINE		VARI_PAUSE			FLAG,3			; VARI PRA RESETAR CIRCUITO NO PAUSE
#DEFINE		VARI_PAUSE_2		FLAG,4			;VARI PRA N ZERAR GASTO NO PAUSE
#DEFINE		VARI_ESTADO_PAUSE	FLAG,5
...
#DEFINE	VARIAVEL_PAUSE		FLAG,6		;PARA TRAVAR OS BOTÕES DURANTE O PAUSE DO CIRCUITO
...
#DEFINE	ESTADO_TIMER		PORTA,0		; SAÍDA PARA BUZZER
...
;MOSTRA MINUTOS, SEGUNDOS E DECIMOS

	MOVF	CONTAGEM_MINUTOS,W		; FAZ O AJUSTE DECIMAL DA INTENSIDADE
	CALL	AJUSTE_DECIMAL			; DO TEMPO
	...
	MOVF	CONTAGEM_SEGUNDOS,W		; FAZ O AJUSTE DECIMAL DA INTENSIDADE
	CALL	AJUSTE_DECIMAL			; DO TEMPO

	BCF		RS						; SELECIONA O DISPLAY P/ COMANDOS
	MOVLW	0X8B					; COMANDO PARA POSICIONAR O CURSOR
	CALL	ESCREVE					; LINHA 1 / COLUNA 1
	BSF		RS						; SELECIONA O DISPLAY P/ DADOS
	...
	MOVF	CONTAGEM_DECIMOS,W	; FAZ O AJUSTE DECIMAL DA INTENSIDADE
	CALL	AJUSTE_DECIMAL		; DO TEMPO

	BCF		RS					; SELECIONA O DISPLAY P/ COMANDOS
	MOVLW	0X8E				; COMANDO PARA POSICIONAR O CURSOR
	CALL	ESCREVE				; LINHA 1 / COLUNA 1
	BSF		RS					; SELECIONA O DISPLAY P/ DADOS
...
;*************************************************************************************
;*							TRATAMENTO DOS BOTÕES DO PROJETO 10						 *
;*************************************************************************************
...
TRATA_BOTAO_2
	...
	CLRF	ACCaHI				; O NÚMERO QUE CONSTA NA DIVISÃO AQUI É O VALOR DE CALIBRAÇÃO POR LITRO
	MOVLW	.1					
	...
	MOVLW	.1					; DIVIDE O VALOR OBTIDO PELO VALOR PRÉ-CALIBRADO
...
LIGA_TIMER							; CHECA SE O TIMER ESTÁ ZERADO
									; CASO ZERADO, O TIMER NÃO LIGA
...
LIGA_TIMER_2					; CASO NÃO ESTEJA ZERADO, CHECA O ESTADO
								; DOS LITROS, ZERANDO A CONTAGEM E INICIANDO A FLAG DE CONTAGEM DE TEMPO
...
LIGA_TIMER_3				; ROTINA DE LIGAÇÃO DO TIMER APÓS A PAUSA. LIMPA AS FLAGS DE PAUSE
...
DECREMENTA_TIMER				;INÍCIO DO TIMER
